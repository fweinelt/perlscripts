...
my $sweep = sweep(
  ...
  instrument => $source,
  from => $div*0.000000100, to => $div*0.000001000, step => $div*0.000000100,
  constant_delay => 1
);

my $amp = 1;
my $delay = $div*0.000015;
my $scal = 1;
my $cycles = 1;
...
$osc->channel_offset(offset => 0.500);
$osc->channel_range(range => 1.5);
$osc->trigger_level(value => 0.02);
...
$sweep->start(
    ...
    datafile_dim => 1,
    point_dim => 1,
    folder => "P16_P13_PWidth_".$sweep->from."s_to_".$sweep->to."s_step_".$sweep->step."s_delay_".$delay."s"
);
