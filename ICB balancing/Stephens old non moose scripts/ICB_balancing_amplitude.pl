...
my $sweep_REFosc = Sweep('SignalRecoveryOscOut', #this sweeps the amplitude step by step
	{
	instrument => $LOCKIN_REF,
	points => [0.01, 0.03], #defines the reference voltage amplitude
	stepwidth => [0.001],
	mode => 'step',
	backsweep => 0,
	delay_before_loop => 5,
...
$sweep_REFosc->add_DataFile($file);
